...
<body style="margin:0;padding:0;background:#f4f6f9;font-family:Arial,Helvetica,sans-serif;">
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08);max-width:600px;width:100%;">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#0a1832 0%,#1e3a5f 100%);padding:32px 40px;text-align:center;">
              <h1 style="margin:0;color:#f5c842;font-size:28px;letter-spacing:-.5px;">Hors-Piste</h1>
          ...
          <tr>
            <td style="padding:40px;">
              ...
              <p style="margin:0;color:#aaa;font-size:12px;text-align:center;">
                Vous recevez cet e-mail car vous êtes inscrit sur Hors-Piste.<br/>
                © ${new Date().getFullYear()} Hors-Piste — paris-sorties.fr
              </p>
            </td>
          </tr>
        </table>
  ...
  try {
    const resendResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Hors-Piste <noreply@paris-sorties.fr>",
        to: [userEmail],
